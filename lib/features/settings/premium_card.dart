...
class _PremiumCardState extends State<PremiumCard> {
  ...
  Widget _manualBox(PremiumVerificationService ps) {
    return Container(
          ...
          child: ElevatedButton.icon(
            onPressed: () async {
              ...
              ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                content: Text(ok ? '✅ تم تفعيل النسخة الاحترافية بنجاح' : '❌ كود التفعيل غير صحيح'),
  ...
  Widget _cloudBox(PremiumVerificationService ps) {
    return Container(
      padding: const EdgeInsets.all(12),
      decoration: BoxDecoration(
        ...
        SizedBox(
          ...
          child: ElevatedButton.icon(
            onPressed: ps.isLoading ? null : () async {
              ...
              if (!mounted) return;
              ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                content: Text(ok ? '✅ تم التفعيل السحابي بنجاح' : '❌ فشل التفعيل السحابي، تحقق من اتصالك'),
                backgroundColor: ok ? Colors.green : Colors.red,
              ));
            },
            icon: ps.isLoading
            ...
            label: Text(ps.isLoading ? 'جاري التفعيل...' : 'تفعيل سحابي',
