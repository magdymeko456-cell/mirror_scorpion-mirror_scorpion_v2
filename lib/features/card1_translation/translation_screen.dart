...
class _TranslationScreenState extends State<TranslationScreen> {
  ...
  static const Map<String, String> _languages = {
    'ar': 'العربية', 'en': 'English', 'fr': 'Francais', 'es': 'Espanol',
    'pt': 'Portugues', 'de': 'Deutsch', 'tr': 'Turkce', 'fa': 'فارسی',
    'ur': 'اردو', 'hi': 'हिन्दी', 'bn': 'বাংলা', 'zh': '中文',
    'ja': '日本語', 'ko': '한국어', 'ru': 'Русский', 'it': 'Italiano',
    'nl': 'Nederlands', 'sv': 'Svenska', 'pl': 'Polski', 'ro': 'Romana',
  };
  ...
  Future<void> _translateText() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (translated != null) setState(() => _translatedController.text = translated);
      }
    } catch (e) {
      setState(() => _translatedController.text = 'خطأ: ${e.toString()}');
    }
      ...
      body: Column(children: [
        Expanded(child: Container(margin: const EdgeInsets.all(8),
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Expanded(child: TextField(controller: _sourceController,
              decoration: const InputDecoration(hintText: 'اكتب النص هنا...', border: InputBorder.none, contentPadding: EdgeInsets.all(8)),
              maxLines: null, expands: true,
            )),
          ]),
        )),
        Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
          SizedBox(width: 150, child: DropdownButtonFormField<String>(value: _selectedLanguage, isExpanded: true,
          ...
          IconButton(icon: Icon(_isRecording ? Icons.mic : Icons.mic_none, color: Colors.teal), onPressed: _startRecording, tooltip: 'إملاء'),
        ]),
        Expanded(child: Container(margin: const EdgeInsets.all(8),
          decoration: BoxDecoration(border: Border.all(color: Colors.grey.shade300), borderRadius: BorderRadius.circular(8)),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Padding(padding: const EdgeInsets.all(8), child: Row(children: [
              Text('الترجمة إلى ${_languages[_selectedLanguage] ?? _selectedLanguage}', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.teal)),
              const Spacer(),
            ...
            Expanded(child: TextField(controller: _translatedController,
              decoration: const InputDecoration(hintText: 'الترجمة ستظهر هنا...', border: InputBorder.none, contentPadding: EdgeInsets.all(8)),
