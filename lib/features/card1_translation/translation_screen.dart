...
class _TextTranslationScreenState extends State<TextTranslationScreen> {
  ...
  void _onSourceChanged(String value) {
    if (_hasTranslated && value.isNotEmpty) {
      ...
      _sourceController.selection = TextSelection.fromPosition(TextPosition(offset: value.length));
  ...
  void _shareAudio() {
    ...
    // Simulate sharing audio file by sharing text with signature as requested
    Clipboard.setData(ClipboardData(text: content));
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('تم نسخ الترجمة مع التوقيع للمشاركة (جاري محاكاة مشاركة الصوت)'),
  ...
  void _copyText() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('تم نسخ النص المترجم بدون توقيع')),
    );
  }
