    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(
          ...
          child: Row(children: [
            Expanded(child: Text(ps.encryptedDeviceId,
                style: const TextStyle(color: Colors.white, fontSize: 11, fontFamily: 'monospace'))),
            IconButton(
              icon: const Icon(Icons.copy, color: Colors.amber, size: 20),
              onPressed: () {
                Clipboard.setData(ClipboardData(text: ps.encryptedDeviceId));
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                onPressed: () async {
                  final ok = await ps.activatePremium(_codeController.text);
                  if (!mounted) return;
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: Text(ok ? '✅ تم التفعيل بنجاح' : '❌ كود غير صحيح'),
        ...
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 8),
            const Text('سيتم إرسال معرف جهازك إلى السيرفر للتفعيل الفوري',
                style: TextStyle(color: Colors.white54, fontSize: 12)),
            const SizedBox(height: 12),
            SizedBox(
              width: double.infinity,
              child: ElevatedButton.icon(
                onPressed: ps.isLoading ? null : () async {
                  final ok = await ps.activateWithServer();
                  if (!mounted) return;
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: Text(ok ? '✅ تم التفعيل السحابي' : '❌ فشل التفعيل'),
                    backgroundColor: ok ? Colors.green : Colors.red,
                  ));
                },
                icon: ps.isLoading
                ...
                label: Text(ps.isLoading ? 'جاري...' : 'تفعيل سحابي',
        ...
        Container(
          ...
          child: const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            SizedBox(height: 10),
            Row(children: [Icon(Icons.email, color: Colors.blue, size: 18), SizedBox(width: 8), Text('إيميل:', style: TextStyle(color: Colors.white54, fontSize: 12))]),
