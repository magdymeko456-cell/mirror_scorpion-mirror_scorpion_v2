#!/bin/bash
set -e
cd ~/mirror_scorpion_translate_version_2 || exit 1

cat > lib/features/card4_stories/stories_screen.dart <<'DART'
import 'dart:convert';
...
echo "=== التحقق: نهاية الملف ==="
tail -1 lib/features/card4_stories/stories_screen.dart
echo "عدد مراجع مكتبات JSON:"
grep -c "assets/data/" lib/features/card4_stories/stories_screen.dart

git add lib/features/card4_stories/stories_screen.dart
git commit -m "feat(stories): load 7 JSON libraries (quran/prophets/hadith/qudsi/arbaeen/asbab) with detail view + TTS + copy, legacy fallback"
git push
